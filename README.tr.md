...
- Provider tasarımı: küçük read interface UTF-8 JSON `byte[]` döner.
- Ana property: `dubbo.provider.service.NestedCatalogService.max-concurrent=16`
- Consumer etkisi: consumer `RawResponse.json(bytes)` ile DTO graph kurmadan döner.

**Typed lookup veya küçük sayfa**

- Provider tasarımı: interface `record`, `String`, primitive, `List<record>` veya `Map<String,String>` döner.
- Ana property: method max-concurrent `4-16`, strict result limit.
- Consumer etkisi: consumer typed business karar verebilir; Hessian/object allocation oluşur.

**DB-backed query**

- Provider tasarımı: `CustomerQueryService` kullanılır, DB pool küçük tutulur.
- Ana property: `sample.db.maximum-pool-size=2`, method max-concurrent `1-2`.
- Consumer etkisi: p99 gizli provider queue yerine DB kapasitesiyle sınırlanır.

**Write command**

- Provider tasarımı: compact JSON command bytes kullanılır.
- Ana property: command method max-concurrent `1`, `sample.db.auto-commit=true`.
- Consumer etkisi: consumer retry kapalı ve saturation anında fail-fast çalışabilir.

**Typed command**

- Provider tasarımı: `CreateCustomerCommand -> CustomerMutationResult` record'ları kullanılır.
- Ana property: command method max-concurrent `1`, Hikari pool ile hizalı.
- Consumer etkisi: daha temiz business contract; byte pass-through'a göre allocation daha fazladır.

**Kubernetes discovery**

- Provider tasarımı: her interface ZooKeeper'a register edilir.
- Ana property: `reactor.dubbo.registry-address=zookeeper://...:2181`
- Consumer etkisi: `zookeeper-discovery` consumer provider restart sonrası reconnect edebilir.

**Lokal/static test**

- Provider tasarımı: `127.0.0.1:20880` veya container DNS bind edilir.
- Ana property: `dubbo.provider.host`, `dubbo.provider.bind-host`, `dubbo.provider.port`
- Consumer etkisi: static provider listesi doğrudan bu provider'a gider.
...
- Ne yapar: ZooKeeper provider URL içine yazılan adres.
- Production notu: consumer pod'ları tarafından erişilebilir olmalı. Gerçek provider discovery için per-pod IP veya headless-service DNS tercih edin. Kubernetes'te `127.0.0.1` publish etmeyin.

**`dubbo.provider.bind-host`**

- Ne yapar: provider'ın dinlediği local interface.
- Production notu: container içinde `0.0.0.0` kullanın.

**`reactor.dubbo.registry-address`**

- Ne yapar: ZooKeeper registry endpoint.
- Production notu: lokal desktop adresi değil Kubernetes DNS kullanın.

**`reactor.dubbo.registry-root`**

- Ne yapar: registry namespace.
- Production notu: consumer `reactor.dubbo.registry-root` ile aynı olmalı.
...
- Neyi kontrol eder: fiziksel PostgreSQL connection sayısı.
- Neden küçük: darboğaz çoğu zaman DB'dir; büyük pool memory ve DB contention artırır.

**`sample.db.minimum-idle=0`**

- Neyi kontrol eder: idle tutulan DB connection sayısı.
- Neden küçük: low-RSS için iyidir; cold DB acquisition p99 bozarsa artırın.

**`CustomerQueryService.max-concurrent`**

- Neyi kontrol eder: provider query bulkhead'i.
- Neden küçük: provider queue değerini DB kapasitesiyle hizalar.

**Method override**

- Neyi kontrol eder: method bazlı hard cap.
- Neden küçük: pahalı tek method'u interface'in tamamını düşürmeden korur.
...
- Neden var: en düşük overhead pass-through JSON.
- Maliyet profili: consumer allocation en düşük; DTO graph yok.

**`String`**

- Sample method: `getCatalogTitle()`, `getCustomerDisplayName(id)`
- Neden var: küçük scalar veri.
- Maliyet profili: küçük allocation ve Hessian decode.

**Primitive**

- Sample method: `countCatalogItems()`, `customerExists(id)`
- Neden var: count ve yes/no lookup.
- Maliyet profili: çok küçük object graph.

**`record`**

- Sample method: `getCatalogInfo()`, `getCustomer(id)`, `getCustomerStats()`
- Neden var: typed business data.
- Maliyet profili: Hessian tek record materialize eder.

**`List<record>`**

- Sample method: `listFeaturedItems(limit)`, `findCustomersBySegment(segment, limit)`
- Neden var: küçük bounded sayfalar.
- Maliyet profili: liste + item record'ları; limit strict olmalı.

**`Map<String,String>`**

- Sample method: `getCatalogAttributes()`
- Neden var: küçük metadata.
- Maliyet profili: Map allocation; hot path'te büyük map kullanmayın.

**`record -> record` command**

- Sample method: `createCustomerTyped(CreateCustomerCommand)`
- Neden var: okunabilir typed command contract.
- Maliyet profili: request object encode + response object decode.

**`byte[] -> byte[]` command**

- Sample method: `createCustomer(byte[])`
- Neden var: en düşük allocation command pass-through.
- Maliyet profili: validation ve JSON response shape provider sorumluluğudur.
...
- Default: `16`
- Neden: küçük sample servisler için güvenli fallback.

**`NestedCatalogService`**

- Property: `dubbo.provider.service.NestedCatalogService.max-concurrent`
- Default: `16`
- Neden: catalog JSON üretimini CPU/allocation açısından sınırlar.

**`NestedCatalogService` typed list/record method'ları**

- Property: `dubbo.provider.service.NestedCatalogService.method.<method>.max-concurrent`
- Default: seçili method'larda `8`
- Neden: tüm interface'i düşürmeden typed DTO/list örneklerini bounded tutar.

**`CustomerQueryService`**

- Property: `dubbo.provider.service.CustomerQueryService.max-concurrent`
- Default: `2`
- Neden: `sample.db.maximum-pool-size=2` ile hizalıdır; DB pool queue büyümesini engeller.
...
- Default: `1`
- Neden: DB-backed method için method-level override örneğidir.

**`CustomerQueryService` typed DB method'ları**

- Property: `dubbo.provider.service.CustomerQueryService.method.<method>.max-concurrent`
- Default: `1-2`
- Neden: record/list/stats method'ları da DB'ye gider; Hikari ile hizalı kalmalıdır.

**`CustomerCommandService`**

- Property: `dubbo.provider.service.CustomerCommandService.max-concurrent`
- Default: `2`
- Neden: write-side DB command concurrency Hikari pool ile hizalı kalır.

**`CustomerCommandService.*` write method'ları**

- Property: `dubbo.provider.service.CustomerCommandService.method.<method>.max-concurrent`
- Default: `1`
- Neden: write örnekleri lokal sample davranışını öngörülebilir tutmak için method bazında serialized tutulur.
...
**Static/nested catalog read**

- Provider interface: `NestedCatalogService`
- Provider darboğazı: CPU/string generation
- Başlangıç limiti: `16`

**PostgreSQL customer read**

- Provider interface: `CustomerQueryService`
- Provider darboğazı: Hikari/PostgreSQL
- Başlangıç limiti: service `2`, method `1`

**Customer create/upsert**

- Provider interface: `CustomerCommandService.createCustomer`
- Provider darboğazı: Hikari/PostgreSQL unique key
- Başlangıç limiti: service `2`, method `1`

**Segment/status patch**

- Provider interface: `CustomerCommandService.patchCustomer*`
- Provider darboğazı: Hikari/PostgreSQL update
- Başlangıç limiti: service `2`, method `1`

**Customer delete**

- Provider interface: `CustomerCommandService.deleteCustomer`
- Provider darboğazı: Hikari/PostgreSQL delete/audit
- Başlangıç limiti: service `2`, method `1`
