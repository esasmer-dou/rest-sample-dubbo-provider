...
Bu sürüm `java-rust-dubbo:0.5.0` kullanır. Provider service interface ve payload sözleşmeleri
değişmedi. `rest-sample-dubbo-consumer:0.4.0` ile doğrudan birlikte çalışır.

Ortak Dubbo service interface'leri `com.reactor.sample:rest-sample-utility:0.3.0` paketinden gelir. Ortak
DTO ve row model record'ları `com.reactor.sample:rust-sample-model:0.3.0` paketinden gelir. Dubbo registry
...
Veri tabanı row record'ları ortak `rust-sample-model` artifact'inden gelir. Bu artifact derlenirken
`SampleCustomerJdbcMapper` ve `CustomerCountsJdbcMapper` üretilir. Repository, tekrar eden
`ResultSet` kodu veya reflection yerine bu doğrudan mapper method'larını çağırır. SQL, transaction,
pool limiti ve business mapping provider kodunda açık kalır.

Provider Maven profilleri fiziksel artifact sınırıdır. `catalog-static-provider`,
`db-query-provider` veya full profili seçin. Kullanılmayan servisleri runtime flag ile saklamak için
tüm provider yüzeyini paketlemeyin. Bu seçim gereksiz Dubbo service ve class yükünü azaltır.
...
| Write command | Compact JSON command bytes | <small><code>CustomerCommandService.max-concurrent=2</code><br>create `2`, aynı satır mutation `1`</small> | İki Hikari connection'ı kullanır; hot-row fan-out'a izin vermez |
| Typed command | `CreateCustomerCommand -> CustomerMutationResult` | <small><code>dubbo.provider.service.CustomerCommandService.method.createCustomerTyped.max-concurrent=2</code><br>Hikari ile hizalı</small> | Daha temiz contract<br>byte pass-through'dan pahalı |
...
- Güvenli PostgreSQL commit davranışı ve `15min / 0.9` checkpoint akışı ile `256MB / 1GB` WAL
  recycle sınırları.
...
Checkpoint değerleri sürekli sample write yükünde dönemsel latency büyümesini azaltır. Commit
güvenliğini zayıflatmaz. `fsync`, `synchronous_commit` ve `full_page_writes` açık kalmalıdır. Harici
PostgreSQL kurulumunda WAL ve checkpoint boyutlandırmasını database ekibi kendi disk gecikmesine ve
recovery süresi hedefine göre yapmalıdır.
...
|---------------------|----------------|--------------|
| `dubbo.provider.service.NestedCatalogService.max-concurrent=16` | `reactor.rust.route-budget.rpc-catalog-read.route-admission.max-concurrent=16` | Basit katalog çağrılarında provider kapasitesiyle aynı başlangıç değeri kullanılabilir. |
| `dubbo.provider.service.CustomerQueryService.max-concurrent=2` | `reactor.rust.route-budget.rpc-customer-raw-read.route-admission.max-concurrent=8` | Kısa async çağrılarda kullanılabilir.<br>p99 yükselirse önce consumer bütçesini düşürün. |
...
## Sınırlı Dubbo/Netty Executor

Provider, Dubbo'nun varsayılan `200` request-handler thread'ini kullanmaz. Sample, export edilen tüm
interface'lerin önünde tek ve sınırlı bir executor kullanır:
...
`eager`, yalnız iş geldiğinde büyür. Idle timeout dolunca tekrar küçülür. `max-threads=8`, thread
stack ve executor memory kullanımını sınırlar. `queue-capacity=16`, kısa burst'leri karşılar fakat
sınırsız latency kuyruğu oluşturmaz. Catalog ve DB interface'leri aynı Dubbo portunu paylaştığı için
executor, Hikari `2` service gate'inden geniştir. Provider RSS, thread sayısı, p99 ve reject oranını
birlikte ölçmeden bu değerleri artırmayın.
...
| Create method'ları | `dubbo.provider.service.CustomerCommandService.method.createCustomer*.max-concurrent` | `2` | Bağımsız customer key'leri için iki Hikari connection'ı da kullanır |
| Patch/delete method'ları | `dubbo.provider.service.CustomerCommandService.method.<method>.max-concurrent` | `1` | Aynı satır lock ve mutation baskısını sınırlar |
...
| Customer create/upsert | `CustomerCommandService.createCustomer` | Hikari/PostgreSQL unique key | <small><code>dubbo.provider.service.CustomerCommandService.max-concurrent=2</code><br><code>dubbo.provider.service.CustomerCommandService.method.createCustomer.max-concurrent=2</code></small> |
...
`service(...)`, interface ile implementation uyumunu startup sırasında doğrular. Bu binding listesi
açık bir başlangıç planıdır; runtime dispatch katmanı değildir. Provider çağrısı export edilen Dubbo
servisine doğrudan gider. Bu refactor hot path'e ek allocation veya lookup maliyeti getirmez.
...
| `dubbo.provider.executor.thread-pool` | Sınırlı Dubbo handler pool tipini seçer. | Low-RSS sample için `eager` kullanın. Sınırsız olduğu için `cached` kabul edilmez. |
| `dubbo.provider.executor.core-threads` | Idle küçülmeden sonra kalacak handler thread sayısıdır. | Tek CPU memory-first podda `1` bırakın. |
| `dubbo.provider.executor.max-threads` | Dubbo handler thread üst sınırıdır. | CPU ve downstream kapasitesi kanıtlanmadan artırmayın. |
| `dubbo.provider.executor.queue-capacity` | Provider request queue üst sınırıdır. | Daha hızlı fail-fast için düşürün; sınırsız queue kullanmayın. |
| `dubbo.provider.executor.idle-timeout-ms` | Fazla eager thread'leri idle kaldıktan sonra küçültür. | Bursty yükte sürekli thread yaratımı görürseniz ölçerek artırın. |
| `dubbo.provider.executor.io-threads` | Netty I/O event-loop thread üst sınırıdır. | Tek CPU sample'da `1` bırakın; ölçmeden artırmayın. |
